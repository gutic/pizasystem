...
switch ($boton) {
  case 'test':
    $num_factura = ulti_factura(2)-1;
    $result = mysqli_query($conexion,"SELECT Id FROM Factura WHERE id_compra = '$num_factura';");
        ...
        for ($i=0; $i < sizeof($tabla_id); $i++) {
          if ($tabla_id[$i] > 0){
            if($tabla_tipo[$i] == 1){
              ...
              VALUES(NULL, '$num_factura','$tabla_id[$i]','$tabla_cant[$i]','$tabla_precio[$i]','2','1');");
            }else {
              ...
              //agregar a detalle factura
              mysqli_query($conexion, "INSERT INTO DetalleFactura (Id, NroComprobante, IdProducto, Cantidad, Precio, tipo_operacion, observacion)
              VALUES(NULL, '$num_factura','$tabla_id[$i]','$tabla_cant[$i]','$tabla_precio[$i]','2','2');");
            }
          };
        };
        $num_factura = ulti_factura(2)-1;
